...
\begin{itemize}
\item \texttt{documentation/FIRST\_PASS\_DIAGNOSIS.md}
\item \texttt{documentation/SECOND\_PASS\_PLAN.md}
\item \texttt{documentation/SECOND\_PASS\_WEBPAGE\_SCAN.md}
\item \texttt{documentation/SECOND\_PASS\_REPO\_NOTES.md}
\item \texttt{documentation/NO\_INTERPOLATION\_DECLARATION.md}
\end{itemize}

\section{First-Pass Diagnosis}
The first-pass diagnosis is recorded in \texttt{documentation/FIRST\_PASS\_DIAGNOSIS.md}. The key findings were:
\begin{itemize}
...
The second-pass webpage scan is recorded in \texttt{documentation/SECOND\_PASS\_WEBPAGE\_SCAN.md}. The most actionable clues from the allowed page were:
\begin{itemize}
...
The second-pass repo notes are recorded in \texttt{documentation/SECOND\_PASS\_REPO\_NOTES.md}. The surviving local repository patterns that mattered most were:
\begin{itemize}
...
\item preserve report generation as a first-class reproducibility artifact under \texttt{documentation/}.
\end{itemize}

\section{Second-Pass Refinement Strategy}
The second-pass plan is recorded in \texttt{documentation/SECOND\_PASS\_PLAN.md}. The implemented refinement strategy was:
\begin{enumerate}
...
Post-processing coverage & The second pass added dedicated scripts for second-pass profiles, \(C_p\), \(C_f\), turbulent shear-stress analogs, and evaluation outputs under \texttt{documentation/}. & Better coverage does not change the flow field directly, but it reduces analysis error by making it easier to compare the same quantities consistently between pass 1 and pass 2. & The second-pass package now contains a more complete and auditable validation set, including graph families analogous to those visible on the allowed webpage. \\
...
\texttt{documentation/*} & Stores the diagnosis, plan, webpage scan, repo notes, no-interpolation declaration, changelog, and this report. \\
\bottomrule
\end{tabular}
\end{table}
...
\item writes machine-readable CSV files under \texttt{documentation/data\_second\_pass};
\item writes graph images under \texttt{documentation/figures\_second\_pass}.
\end{itemize}
...
\end{itemize}
The second-pass figure package also copies the available ParaView renders into \texttt{documentation/figures\_second\_pass} so the PDF can present both plot-based validation and visual field inspection.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{figures_second_pass/paraview_velocity.png}
\caption{ParaView velocity rendering from the refreshed second-pass VTK output.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{figures_second_pass/paraview_pressure.png}
\caption{ParaView pressure rendering from the refreshed second-pass VTK output.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{figures_second_pass/paraview_streamlines.png}
\caption{ParaView streamline rendering from the refreshed second-pass VTK output.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{figures_second_pass/paraview_wall_shear.png}
\caption{ParaView wall-shear rendering from the refreshed second-pass VTK output.}
\end{figure}
...
The second-pass score and delta are reported from \texttt{documentation/data\_second\_pass/reconstructed\_nasa\_score\_second\_pass.json}. The refined setup was actually worse than the first pass at the 50-iteration write, slightly better at the 100-iteration write, and substantially better by the final 300-iteration field. That behavior is a useful reminder that this case should be judged from a genuinely converged field rather than the earliest available write.
...
The final residuals from \texttt{documentation/data\_second\_pass/second\_pass\_summary.json} were:
\begin{itemize}
