#!/usr/bin/env python3
"""Generate pass-five NASA hump figures and summary plots."""
...
OUT_DATA = ROOT / "documentation_src" / "data_pass5"
OUT_FIG = ROOT / "documentation_src" / "figures_pass5"
...
def build_cp_cf_plot(wall: dict[str, np.ndarray | float | None]) -> dict[str, float]:
    ...
    with (OUT_DATA / "cp_comparison_pass5.csv").open("w", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(["x_over_c", "Cp_exp", "Cp_cfd", "Cp_error"])
        writer.writerows(cp_rows.tolist())

    with (OUT_DATA / "cf_comparison_pass5.csv").open("w", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(["x_over_c", "Cf_exp", "Cf_uncertainty", "Cf_cfd", "Cf_error"])
        writer.writerows(cf_rows.tolist())
    ...
    axes[0].scatter(cp_exp[:, 0], cp_exp[:, 1], color="#d62728", s=18, label="NASA experiment")
    axes[0].set_ylabel("Cp")
    axes[0].set_title(f"PASS 5 Cp comparison, RMSE={cp_rmse:.4f}, MAE={cp_mae:.4f}")
    axes[0].grid(True, alpha=0.3)
    axes[0].legend()

    axes[1].plot(x_over_c, cf_cfd, color="#1f77b4", linewidth=2.0, label="OpenFOAM")
    axes[1].errorbar(cf_exp[:, 0], cf_exp[:, 1], yerr=cf_exp[:, 2], fmt="o", color="#d62728", markersize=3.5, capsize=2.0, label="NASA experiment")
    axes[1].axhline(0.0, color="black", linewidth=0.8, alpha=0.6)
    axes[1].set_xlabel("x/c")
    axes[1].set_ylabel("Cf")
    axes[1].set_title(f"PASS 5 Cf comparison, RMSE={cf_rmse:.5f}, MAE={cf_mae:.5f}")
    axes[1].grid(True, alpha=0.3)
    axes[1].legend()
    fig.tight_layout()
    fig.savefig(OUT_FIG / "cp_cf_vs_nasa_pass5.png", dpi=200)
    plt.close(fig)
...
def build_experiment_summary_plot() -> None:
    summary = json.loads((OUT_DATA / "pass5_experiment_summary.json").read_text())
    rows = [row for row in summary["rows"] if row.get("mae") is not None and row.get("cp_mae") is not None and row.get("cf_mae") is not None]
    labels = [f"{row['tag']}@{row['time']}" for row in rows]
    mae = np.array([row["mae"] for row in rows], dtype=float)
    cp_mae = np.array([row["cp_mae"] for row in rows], dtype=float)
    cf_mae = np.array([row["cf_mae"] for row in rows], dtype=float)

    fig, axes = plt.subplots(3, 1, figsize=(11, 10), sharex=True)
    axes[0].bar(labels, mae, color="#1f77b4")
    axes[0].set_ylabel("Benchmark MAE")
    axes[0].grid(True, axis="y", alpha=0.3)

    axes[1].bar(labels, cp_mae, color="#ff7f0e")
    axes[1].set_ylabel("Cp MAE")
    axes[1].grid(True, axis="y", alpha=0.3)

    axes[2].bar(labels, cf_mae, color="#2ca02c")
    axes[2].set_ylabel("Cf MAE")
    axes[2].grid(True, axis="y", alpha=0.3)
    axes[2].tick_params(axis="x", rotation=35)

    fig.tight_layout()
    fig.savefig(OUT_FIG / "pass5_experiment_metrics.png", dpi=200)
    plt.close(fig)
...
def main() -> None:
    ...
    metrics = build_cp_cf_plot(wall)
    build_experiment_summary_plot()
    summary = {
        ...
        "figure_file": "documentation_src/figures_pass5/cp_cf_vs_nasa_pass5.png",
        "experiment_summary_file": "documentation_src/data_pass5/pass5_experiment_summary.json",
    }
    (OUT_DATA / "pass5_comparison_summary.json").write_text(json.dumps(summary, indent=2))
